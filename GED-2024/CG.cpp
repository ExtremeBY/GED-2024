...
void CG::Generator::Head()
{
	...
	out << "includelib ../Debug/GED_Lib.lib\n";
...
void CG::Generator::Code()
{
	...
	for (int i = 0; i < lextable.size; i++)
	{
		...
		case LEX_IF: {
			...
			auto iterations = lextable.table[i++];
			
			out << "\tpush ";
			out << idtable.table[lhs.idxTI].scope << idtable.table[lhs.idxTI].id << "\n";
			// Записываем метку для начала цикла

			std::string while_mark_start = "while_start" + std::to_string(if_mark_count);
			std::string while_mark_end = "while_end" + std::to_string(if_mark_count++);
			out << while_mark_start << ":\n";
			if_marks.push(while_mark_start);
			if_marks.push(while_mark_end);

			out << "\tcmp "<< idtable.table[lhs.idxTI].scope << idtable.table[lhs.idxTI].id <<", 0\n";
			out << "\tjz " << if_marks.top() << "\n"; // Здесь метка для выхода из цикла
			...
			for (int j = i; j < i+2; j++)
			{
				...
				{
				case LEX_PRINT: {
					if (lextable.table[j + 1].lexema == LEX_ID) {
						if (idtable.table[lextable.table[j + 1].idxTI].iddatatype == IT::IDDATATYPE::INT) {
							out << "\tpush\t\t" << idtable.table[lextable.table[j + 1].idxTI].scope
								<< idtable.table[lextable.table[j + 1].idxTI].id;
							out << "\n\tcall\t\t_ConsoleWriteInt\n";
						}
						else {
							out << "\tpush\t\t" << idtable.table[lextable.table[j + 1].idxTI].scope
					...
					else if (lextable.table[j + 1].lexema == LEX_LITERAL) {
						if (idtable.table[lextable.table[j + 1].idxTI].iddatatype == IT::IDDATATYPE::INT) {
							out << "\tpush\t\t" << idtable.table[lextable.table[j + 1].idxTI].literalID;
							out << "\n\tcall\t\t_ConsoleWriteInt\n";
						}
						else {
							out << "\tpush\t\toffset " << idtable.table[lextable.table[j + 1].idxTI].literalID;
							out << "\n\tcall\t\toutput\n\n";
						}
					}
					break;
				...
				case LEX_EQUAL: {
					if (lextable.table[j + 1].lexema == LEX_LITERAL) {
						if (idtable.table[lextable.table[j + 1].idxTI].iddatatype == IT::IDDATATYPE::INT)
							out << "\tpush\t\t";
						else
							out << "\tpush\t\toffset ";
						out << idtable.table[lextable.table[j + 1].idxTI].literalID << "\n";
					}
					if (lextable.table[j + 1].lexema == LEX_PLUS)
					{
						out << "\t;\\/(ADD)+\\/\n";
						out << "\tpop\t\teax\n";
						out << "\tpop\t\tebx\n";
			...
			out << "\tsub "<< idtable.table[lhs.idxTI].scope << idtable.table[lhs.idxTI].id <<", 1\n";;
			out << "\tjmp " << while_mark_start << "\n"; // Переход к началу цикла
			out << while_mark_end << ":\n";
			break;
		}
